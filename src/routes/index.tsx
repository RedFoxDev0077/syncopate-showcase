    ...
    <main>
        ...
        <div className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
            ...
            {PROFILE.stats.map((stat) => (
              <div key={stat.label}>
                ...
                <dd>
                  ...
                  <span className="mt-1 block text-sm text-ink-foreground/60">{stat.label}</span>
                </dd>
              </div>
            ))}
          </dl>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            ...
            <p className="text-sm text-ink-foreground/60">
              {PROFILE.location} · {PROFILE.rate} · {PROFILE.languages}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-20">
        <div className="grid gap-12 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            <h3 className="mt-10 text-lg font-semibold">What I ship</h3>
          ...
          <aside className="h-fit space-y-8 rounded-2xl border border-border bg-muted p-6">
            <div>
              <h3 className="text-sm font-semibold">Core skills</h3>
              <dl className="mt-3 space-y-2 text-sm">
                {PROFILE.coreSkills.map((skill) => (
                  <div key={skill.name} className="flex justify-between gap-4">
                    ...
                    <dd className="text-muted-foreground">{skill.experience}</dd>
                  </div>
                ))}
              </dl>
            </div>
      ...
      <section className="border-y border-border bg-muted/40">
        <div className="mx-auto max-w-6xl px-6 py-16">
          <h2 className="text-2xl font-semibold tracking-tight">What I work with</h2>
          <div className="mt-8 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {PROFILE.stack.map((group) => (
              <div key={group.area}>
                <h3 className="text-sm font-semibold text-primary">{group.area}</h3>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              A few recent projects — {PROJECTS.length} in total.
            </p>
          </div>
          <Link
            to="/projects"
        ...
        <div className="mx-auto max-w-6xl px-6 py-20">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="inline-flex items-center gap-1 text-sm text-muted-foreground">
              ...
              {REVIEW_SUMMARY.rating} · {REVIEW_SUMMARY.count} reviews
            </span>
          </div>
            ...
            {TESTIMONIALS.filter((t) => t.quote).map((testimonial) => (
              ...
              >
                ...
                <figcaption className="mt-6 border-t border-border pt-4 text-sm">
                  ...
                  <span className="mt-1 block text-muted-foreground">{testimonial.project}</span>
                </figcaption>
              </figure>
            ))}
          </div>
